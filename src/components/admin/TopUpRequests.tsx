...
const TopUpRequests = () => {
  ...
  const filteredRequests = (() => {
    ...
    return (requests ?? []).filter((request: any) => {
      const matchesSearch =
        ...
        norm(request.agent_name).includes(q) ||
        norm(request.agent_email).includes(q) ||
      ...
      const matchesAgent =
        !agentFilter || norm(request.agent_name).includes(norm(agentFilter));
  ...
  const uniqueBanks = [
    ...new Set(requests.map((r) => r.bank_name).filter(Boolean)),
  ];
